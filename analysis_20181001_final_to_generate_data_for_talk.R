...
dyn.load('cpp_files/reg_20180312_TMB_example.dll')
...
# Objective function in tensorflow ----------------------------------------
library(tensorflow)
tfe_enable_eager_execution() 

## auxilliary functions
get_nchoosek_tf <- function(n, k){
    
    return(tf$exp(tf$lgamma(n + 1) - tf$lgamma(n - k + 1) - tf$lgamma(k + 1)))
    
}


get_prob_of_scoreline_tf <- function(me, you, prob_win_point){
    ...
    out_prob <-
        ## if game goes past deuce
        # binomial pmf for 20 successes out of 40 trials
        tf$to_float(tf$math$greater_equal(you, 20)) *  
        get_nchoosek_tf(40, 20) * 2^(you - 20) * prob_win_point^me * (1 - prob_win_point)^you+
        ## otherwise a normal game
        (1 - tf$to_float(tf$math$greater_equal(you, 20))) *
        get_nchoosek_tf(20 + you, you) * prob_win_point^me * (1 - prob_win_point)^you
...
optim_in_tf <- list()
optim_in_tf$fn <- function(par) as.numeric(obj_fn_tf(par[1], par[2], par[3], par[4]))
optim_in_tf$gr <- function(par) get_grad_tf(par[1], par[2], par[3], par[4])
optim_in_tf$par <- c(0.592246, 0, 0, 0)
optim_in_tf$method <- 'BFGS'


(optim_out_tf <- do.call(optim, optim_in_tf))
